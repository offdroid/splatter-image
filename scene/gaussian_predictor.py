...
class GaussianSplatPredictor(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        assert (
            cfg.model.network_with_offset or cfg.model.network_without_offset
        ...
        if cfg.model.network_with_offset:
            ...
            self.network_with_offset = networkCallBack(
                cfg,
                cfg.model.name,
                split_dimensions,
                scale=scale_inits,
        ...
        if cfg.model.network_without_offset:
            ...
            self.network_wo_offset = networkCallBack(
                cfg,
                cfg.model.name,
                split_dimensions,
                scale=scale_inits,
        ...
        if self.cfg.cam_embd.embedding is not None:
            if self.cfg.cam_embd.encode_embedding is None:
                ...
                self.cam_embedding_map = PositionalEmbedding(
                    self.cfg.cam_embd.dimension
                )
    ...
    def init_ray_dirs(self):
        ...
        if self.cfg.data.category not in ["hydrants", "teddybears"]:
            ray_dirs[:, :2, ...] /= fov2focal(
                self.cfg.data.fov * np.pi / 180, self.cfg.data.training_resolution
            )
    ...
    def get_camera_embeddings(self, cameras):
        ...
        if self.cfg.cam_embd.embedding == "pose":
            ...
            cam_embedding = torch.cat(
                [cameras[:, :, 3, :3], cameras[:, :, 2, :3]], dim=2
            )

        cam_embedding = rearrange(cam_embedding, "b n_view c -> (b n_view) c")
        cam_embedding = self.cam_embedding_map(cam_embedding)
        cam_embedding = rearrange(
            cam_embedding, "(b n_view) c -> b n_view c", b=b, n_view=n_view
        )
    ...
    def get_pos_from_network_output(
        ...
        ray_dirs_xy = self.ray_dirs.expand(
            depth_network.shape[0], 3, *self.ray_dirs.shape[2:]
        )
        if self.cfg.data.category in ["hydrants", "teddybears"]:
            assert torch.all(focals_pixels > 0)
            ray_dirs_xy = ray_dirs_xy.clone()
            ray_dirs_xy[:, :2, ...] = ray_dirs_xy[:, :2, ...] / focals_pixels.unsqueeze(
                2
            ).unsqueeze(3)
        ...
        if const_offset is not None:
            depth = (
                self.depth_act(depth_network)
                * (self.cfg.data.zfar - self.cfg.data.znear)
                + self.cfg.data.znear
                + const_offset
            )
        else:
    ...
    def forward(
        ...
        source_cameras_view_to_world = source_cameras_view_to_world.reshape(
            B * N_views, *source_cameras_view_to_world.shape[2:]
        )
        x = x.contiguous(memory_format=torch.channels_last)

        if self.cfg.model.network_with_offset:

            split_network_outputs = self.network_with_offset(
                x, film_camera_emb=film_camera_emb, N_views_xa=N_views_xa
            )
            ...
            pos = self.get_pos_from_network_output(
                depth, offset, focals_pixels, const_offset=const_offset
            )

        else:
            split_network_outputs = self.network_wo_offset(
                x, film_camera_emb=film_camera_emb, N_views_xa=N_views_xa
            ...
            pos = self.get_pos_from_network_output(
                depth, 0.0, focals_pixels, const_offset=const_offset
            )

        if self.cfg.model.isotropic:
            scaling_out = torch.cat(
                [scaling[:, :1, ...], scaling[:, :1, ...], scaling[:, :1, ...]], dim=1
            )
        else:
            scaling_out = scaling
        ...
        if activate_output:
            ...
            out_dict["scaling"] = self.flatten_vector(
                self.scaling_activation(scaling_out)
            )
        else:
        ...
        source_cv2wT_quat = source_cv2wT_quat.reshape(
            B * N_views, *source_cv2wT_quat.shape[2:]
        )
        out_dict["rotation"] = self.transform_rotations(
            out_dict["rotation"], source_cv2wT_quat=source_cv2wT_quat
        )
        ...
        if self.cfg.model.max_sh_degree > 0:
            ...
            out_dict["features_rest"] = features_rest.reshape(
                *features_rest.shape[:2], -1, 3
            )
            assert self.cfg.model.max_sh_degree == 1  # "Only accepting degree 1"
            out_dict["features_rest"] = self.transform_SHs(
                out_dict["features_rest"], source_cameras_view_to_world
            )
        else:
            out_dict["features_rest"] = torch.zeros(
                (
                    out_dict["features_dc"].shape[0],
                    out_dict["features_dc"].shape[1],
                    (self.cfg.model.max_sh_degree + 1) ** 2 - 1,
                    3,
                ),
                dtype=out_dict["features_dc"].dtype,
                device="cuda",
            )
