...
def evaluate_dataset(
    ...
    for d_idx, (data, _, input_data) in enumerate(tqdm.tqdm(dataloader)):
        ...
        if (
            model_cfg.data.category == "hydrants"
            or model_cfg.data.category == "teddybears"
        ):
...
def eval_robustness(model, dataloader, device, model_cfg, out_folder=None):
    ...
    resize_to_128_transform = transforms.Resize(
        128, interpolation=transforms.InterpolationMode.BILINEAR
    )
    ...
    for test_zoom_idx, crop_size in enumerate([-40, -30, -20, -10, 0, 10, 20, 30, 40]):
        ...
        if crop_size >= 0:
            ...
            padding_transform = transforms.Pad(padding=-crop_size, fill=1.0)
            input_images = padding_transform(
                data["gt_images"][0, : model_cfg.data.input_images]
            )
        ...
        padding_transform = transforms.Pad(
            padding=(abs(x_shift), abs(y_shift)), fill=1.0
        )

        padded_source = padding_transform(
            data["gt_images"][0, : model_cfg.data.input_images]
        )
        ...
        out_example_gt = os.path.join(
            out_folder, "{}_".format(test_zoom_idx) + example_id + "_gt"
        )
...
@torch.no_grad()
def main(
    dataset_name, experiment_path, device_idx, split="test", save_vis=0, out_folder=None
):
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the parent folder of the model. \
    ...
    parser.add_argument(
        ...
        type=str,
        default="out",
        help="Output folder to save renders (default: out)",
    )
    parser.add_argument(
        "--save_vis",
        type=int,
...
if __name__ == "__main__":
    ...
    if split == "vis":
        print(
            "Will not print or save scores. Use a different --split to return scores."
        )
    ...
    if split != "vis":
        if experiment_path is not None:
            score_out_path = os.path.join(
                experiment_path, "{}_scores.json".format(split)
            )
        else:
