...
class FullyConnectedLayer(torch.nn.Module):
    def __init__(
        ...
        self.weight = torch.nn.Parameter(
            torch.randn([out_features, in_features], device=device) / lr_multiplier
        )
        self.bias = (
            torch.nn.Parameter(
                torch.full([out_features], np.float32(bias_init), device=device)
            )
            if bias
            else None
        )
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(
        ...
        self.out = FullyConnectedLayer(
            in_channels, 1 if cmap_dim == 0 else cmap_dim, device=device
        )
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        if self.add_camera_cond:
            if (
                ...
                or self.data_camera_mode == "shapenet_chair"
                or self.data_camera_mode == "shapenet_motorbike"
        ...
        common_kwargs = dict(
            img_channels=self.img_channels_drgb,
    ...
    def forward(
        self, img, c, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs
    ):
        ...
        for res in self.block_resolutions:
            ...
            x, img_for_tex = block(
                x, img_for_tex, alpha, (img_res // 2) == res, **block_kwargs
            )
        cmap = None
        if self.c_dim > 0:
